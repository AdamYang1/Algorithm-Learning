...
\begin{itemize}
		\item Assume the matching $\mu^*$ is optimal and different from $\mu$ (algo). Then, $\exists$ smallest $i$ S.T. $\mu^*(i) \neq \mu(i) = i$
            ...
            \item Other way: [Fractional Knapsack] algo: $a_i<a_j$ opt: $o_i<o_j$, $a_i=o_i+\epsilon, a_j=o_j-\epsilon*X$. Derive $X=\frac{W_i}{W_j}$ for calculating the equal total sum.
		\item Thus, a\textbf{fter the swap}, the total cost of the swap would decrease or stay the same.
		\item As $\mu^*$ is the optimal solution it must be the case that the cost stays the same, however now the first $i$ positions of $\mu^*$ match $\mu$. Repeating this process for all $n$ positions will lead to a matching $\mu^*$ equal to $\mu^*$. This concludes $\mu$ is also an optimal matching.
\end{itemize}
...
\[
		T(n) = 3T(n /2) + \Theta (n)
\] 
which by (MT1) has $T(n) = \Theta (n^{\log _3 2})$.
...
\textbf{Key insight}: 
\begin{itemize}
		\item If optimal solution includes interval $i$, then it does not include anything else intersecting $i$.
		\item If optimal solution does not include $i$, it is the same as the optimal solution for intervals $1,..., i-1, i+1, ..., n$.
		\item
