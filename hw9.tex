...
\qquad 1. "Yes" instances map to "Yes" instances.
...
\qquad 2. "No" instances map to "No" instances.
...
\qquad 3. $f$ runs in polynomial time.

Here are proofs for properties listed above:

\qquad 1: If correct answer for $x$ is "Yes", then there is a binary string $y$ of length at most $|y| \leqslant r(|x|) = |\hat{z}|$ S.T. $B(x,y)$ return "Yes" in $|\hat{t}| = q(|x|+|y|)$ [properties discussed in lectures]. Therefore correct answer for $(B, x, \hat{t}, \hat{z})$ is "Yes" now that $P(\hat{x},y) = B(x,y)$ terminates in $|\hat{t}| = q(|x|+|y|)$ steps.
...
\qquad 3: $|$binary string$|$ is polynomial. Adding $B$ takes $\mathcal{O}(1)$ time. Therefore, $f$ runs in polynomial time.
