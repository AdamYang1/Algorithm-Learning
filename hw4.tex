...
\begin{algorithm}[!ht]
\DontPrintSemicolon
\KwInput{$a[ ]$,$b[]$, $l_a$, $u_a$, $l_b$, $u_b$ (indexed from 0 to n-1)}
  \SetKwFunction{FSub}{FindStudent} 
    \SetKwProg{Fn}{Def}{:}{} 
      \Fn{\FSub{$a[ ]$,$b[]$, $l_a$, $u_a$, $l_b$, $u_b$}}{ 
        \If{$u_a+u_b-l_a-l_b == 2$ or $4$}{
            sort $a[l],a[u],b[l],b[u]$
            
            return the median of these numbers
        } \Else{
            $m_a \leftarrow \lfloor(u_a+l_a)/2 \rfloor$
            
            $m_b \leftarrow \lfloor(u_b+l_b)/2 \rfloor$
