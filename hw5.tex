...
\begin{proof}[Algorithm]{}{An Algorithm to calculate the sequence of die rolls gets to winning square as quickly as possible.}
    ...
    % \texttt{bool hasNext(int[] dp, int index)}\\
    % \qquad \texttt{}

    \texttt{def findSeq(d[]) //d[] is the map information array}

    \texttt{/** \\ If i is a ladder, d[i] = a square later in the board.\\If i is a chute, d[i] = undefined because you die. \\If i is a normal square, d[i] = i. \\ **/}
    
    \qquad \texttt{int dp[n+1]; //dp[i] is the min step to get to index i}

    \qquad \texttt{dp[0]=0;}

    \qquad \texttt{for(int i = 1; i <= n+1; ++i) \{}
